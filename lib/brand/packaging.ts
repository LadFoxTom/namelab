...
import { LogoVariations } from './variationGenerator';
...
interface PackageAssets {
  ...
  // Logo variation system (6 variations x 5 color versions)
...
export async function assembleZip(assets: PackageAssets): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    // Logo variation system: 6 types x 5 color versions
    if (assets.logoVariations) {
      const v = assets.logoVariations;
      const variationSets = [v.horizontalLockup, v.stackedLockup, v.icon, v.wordmark, v.monogram, v.taglineLockup];
      for (const set of variationSets) {
        for (const file of set.variations) {
          archive.append(file.buffer, { name: `${name}/logo/${set.name}/${file.filename}` });
        }
      }
    }
...
`- logo/ — Full logo system: primary lockup, stacked, icon, wordmark, monogram, tagline (each in 5 color versions) + SVG + hi-res
- favicons/ — All sizes + .ico + webmanifest
